...
Wayne_county <- get_acs(geography = "block group", variables = "B01003_001", 
                        state = "MI", county = "Wayne", geometry = TRUE)


Macomb_county <- get_acs(geography = "block group", variables = "B01003_001", 
                         state = "MI", county = "Macomb", geometry = TRUE)


Oakland_county <- get_acs(geography = "block group", variables = "B01003_001", 
...
counties_df$CRIME_PER_PERSON <- (counties_df$n/counties_df$estimate)/4
...
vars <- c("B01001_007","B01002_001","B01003_001")
...
counties_df2$variable <- case_when(
    ...
    counties_df2$variable == "B01003_001" ~ "Population",
)
...
df2 <- df2 %>% mutate(Crime_Type = case_when(
    ...
    .$Crime == "SEX OFFENSES" ~ "Sex Related",
    .$Crime == "SEXUAL ASSAULT" ~ "Sex Related",
    .$Crime == "FRAUD" ~ "Fraud",
    .$Crime == "FORGERY" ~ "Fraud",
    TRUE ~ "Other"
))
...
ui <- navbarPage("Detroit Crime and Demographic Analysis",
                 tabPanel("Crimes Tab",
...
server <- function(input, output) {
    ...
    output$crime_dots <- renderPlotly({
        ...
        p <- plot_mapbox(mode = 'scattermapbox') %>%
            add_markers(
                data = df2, x = ~longitude, y = ~latitude, text=~hover, color=~Crime, size=1, hoverinfo = "text", alpha = 0.7) %>%
            layout(
