...
ui <- navbarPage("Detroit Crime Map",
                 tabPanel("Map",
                          selectInput("Crime", "Crime:", options, width = "100%",multiple = TRUE),
                          # sliderInput("Date",
                          #             "Dates:",
...
server <- function(input, output) {
    ...
    output$map <- renderPlotly({
        ...
        df <- df  %>%
            filter(Crime == input$Crime) %>% filter(Date > input$Date[1]) %>% filter(Date < input$Date[2])
        
        
        p <- plot_mapbox(mode = 'scattermapbox') %>%
            add_markers(
                data = df, x = ~longitude, y = ~latitude, text=~hover, color=~Crime, size=1, hoverinfo = "text", alpha = 0.5) %>%
            layout(
