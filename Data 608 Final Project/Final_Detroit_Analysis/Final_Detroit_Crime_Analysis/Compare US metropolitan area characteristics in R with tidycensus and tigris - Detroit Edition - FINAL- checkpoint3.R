...
crime_with_geo <- counties_df %>% select(GEOID,NAME,geometry) %>% inner_join(crimes_by_GEOID)
...
counties_df %>% filter(variable=="population") %>% filter(estimate>0) %>% mapview(zcol = "estimate", legend = TRUE)
mapview_df %>% filter(variable=="SEX OFFENSES") %>% mapview(zcol = "estimate", legend = TRUE)
